...
const Listing = () => {
  ...
  const [contactLandlor, setContactLandlord] = useState(false)
  ...
  return (
    <main>
      ...
      <div className="m-4 flex flex-col md:flex-row max-w-6xl lg:mx-auto p-4 rounded large border-3 shadow-lg lg:space-x-5">
        <div className="w-full">
          <p className="text-2xl font-bold mb-3 text-blue-900">
            {listing.name} - $ {listing.offer ? listing.discountPrice.toString()
          ...
          <ul className="flex items-center space-x-2 sm:space-x-5 lg:space-x-10 text-sm font-semibold mb-6">
          ...
          {listing.userRef !== auth.currentUser?.uid && !contactLandlor && (
            <div className="mt-6">
              <button onClick={()=>setContactLandlord(true)} className="px-7 py-3 bg-blue-600 text-white font-medium text-sm uppercase rounded shadow-bd hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg w-full text-center transition duration-150 ease-in-out">Contact Landlord</button>
            </div>
          )}
          {contactLandlor && (
